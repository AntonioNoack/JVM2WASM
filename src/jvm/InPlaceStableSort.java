...
class InPlaceStableSort {
    ...
    private static <V> int lower(int from, int to, V val, V[] array, Comparator<V> c) {
        int len = to - from, half;
        while (len > 0) {
            half = len >> 1;
            final int mid = from + half;
            if (c.compare(array[mid], val) < 0) {
                from = mid + 1;
                len = len - half - 1;
            } else len = half;
        }
    ...
    private static <V> int upper(int from, int to, V val, V[] array, Comparator<V> c) {
        int len = to - from, half;
        while (len > 0) {
            half = len >> 1;
            final int mid = from + half;
            if (c.compare(val, array[mid]) < 0) {
                len = half;
            } else {
                from = mid + 1;
                len = len - half - 1;
    ...
    private static <V> void insertSort(int from, int to, V[] array, Comparator<V> c) {
        // check bounds first, and then we can use unsafe element access
        if (from < 0 || c == null || array.length < to) {
            throw new IllegalArgumentException();
        }
        // it would be nice if we could resolve the method call for c right here
        int addr = getAddr(array) + arrayOverhead;
    ...
    private static <V> void rotate(int from, int mid, int to, V[] array) {
    ...
    private static <V> void merge(int from, int pivot, int to, int len1, int len2, V[] array, Comparator<V> c) {
        ...
        if (len1 > len2) {
            len11 = len1 >> 1;
            cut1 = from + len11;
            cut2 = lower(pivot, to, array[cut1], array, c);
            len22 = cut2 - pivot;
        } else {
            len22 = len2 >> 1;
            cut2 = pivot + len22;
            cut1 = upper(from, pivot, array[cut2], array, c);
            len11 = cut1 - from;
        }
        rotate(cut1, pivot, cut2, array);
        final int new_mid = cut1 + len22;
        merge(from, cut1, new_mid, len11, len22, array, c);
        merge(new_mid, cut2, to, len1 - len11, len2 - len22, array, c);
    }
