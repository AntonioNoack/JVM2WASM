...
fun findDependencies(functions: List<FunctionImpl>, clusters: List<Clustering>) {
 ...
 * */
fun collectNodes(cluster: Cluster, targetComplexity: Int, dst: ArrayList<Clustering>) {
...
fun getComplexity(function: FunctionImpl): Int {
    return getComplexity(function.body)
}

fun getComplexity(instr: Instruction): Int {
    return when (instr) {
...
fun getComplexity(instr: List<Instruction>): Int {
    return instr.sumOfInt(::getComplexity)
}

fun addComplexity(node0: Cluster, complexity: Int) {
    var node = node0
    while (true) {
...
class Cluster(val name: String, val parent: Cluster?) {
...
fun getCluster(root: Cluster, path: List<String>): Cluster {
    var node = root
    for (i in 0 until path.lastIndex) {
        val name = path[i]
        if (name.isBlank2()) continue
        node = node.children.getOrPut(name) { Cluster(name, node) }
    }
    return node
}
